...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function getTasksFromAPI() {
        const response = await fetch(API_URL, {
            headers: { Authorization: `Token 120e859567d04fc11a519777ff5148cd211fb366` }
        });
    ...
    async function saveData() {
        ...
        if (!title) {
            swal({ title: "Error", text: "Please enter a task title!", icon: "error" });
            return;
        }
    ...
    if (addButton) addButton.addEventListener("click", saveData);
    ...
    const displayTasks = async (section) => {
        ...
        switch (section) {
            case "thisWeek":
                ...
                filtered = filtered.filter((t) => t.deadline && new Date(t.deadline) >= start && new Date(t.deadline) <= end);
                break;
            case "thisMonth":
                titleLink.textContent = "Current Month";
                filtered = filtered.filter((t) =>
                    t.deadline && new Date(t.deadline).getMonth() === new Date().getMonth()
                );
                break;
            default:
            ...
            taskContainer.innerHTML = filtered
                .map((task) => {
                    ...
                    const dateLabel = task.deadline
                        ? (isToday(task.deadline) ? "Today" : `<i class='bx bx-calendar-alt'></i> ${task.deadline.replace("T", " ")}`)
                        : "";
